  ...
  return (
    <div className="fixed inset-0 z-[60] bg-bg overflow-hidden">
      ...
      <div className="h-full w-full flex" style={{ padding: 'clamp(28px, 3vw, 48px)' }}>
        ...
        {/* 좌측: 대사 영역 (Rebuttal은 반대쪽) */}
        <div className={`flex-1 flex flex-col justify-center ${mounted ? 'animate-fade-slide-up stagger-1' : 'opacity-0'}`}
          style={{ paddingLeft: 'clamp(20px, 3vw, 40px)', paddingRight: 'clamp(40px, 5vw, 80px)' }}>

          {/* 타이틀 */}
          <div style={{ marginBottom: 'clamp(24px, 3vw, 48px)' }}>
            <span className="label-caps text-ink/15" style={{ fontSize: '0.5rem', letterSpacing: '0.2em' }}>REBUTTAL</span>
        ...
        {/* 우측: 캐릭터 이미지 + 부위 */}
        <div className={`flex-1 flex items-center justify-center ${mounted ? 'animate-fade-slide-up stagger-2' : 'opacity-0'}`}>
          {data?.characterImage ? (
            <div className="relative max-h-[80vh] max-w-[40vw]">
              <img src={data.characterImage} alt="Sakdāgāmi" className="max-h-[80vh] max-w-full object-contain" />
      ...
      {/* 하단 페이지 번호 */}
      <div className={`fixed bottom-0 left-0 right-0 flex justify-start px-8 ${mounted ? 'animate-fade-slide-up stagger-4' : 'opacity-0'}`}
        style={{ paddingBottom: 'clamp(12px, 1.5vw, 20px)' }}>
        <div className="flex items-center gap-2">
          <span className="label-caps text-ink/25" style={{ fontSize: '0.5rem', letterSpacing: '0.15em' }}>R</span>
          <span className="text-ink/15" style={{ fontSize: '0.45rem' }}>/</span>
          <span className="heading-condensed text-ink/25" style={{ fontSize: '0.55rem', fontStyle: 'italic' }}>Rebuttal</span>
        </div>
      </div>
    </div>
  )
